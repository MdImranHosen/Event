...
// Admin Login Script......
  ...
  public function admin_eventcheck($admin_email,$admin_pass,$admin_type,$admin_event) {

        $admin_email = $this->fm->validation($admin_email);
        $admin_pass  = $this->fm->validation($admin_pass);
        $admin_type  = $this->fm->validation($admin_type);
        $admin_event = $this->fm->validation($admin_event);
        
        $admin_email = mysqli_real_escape_string($this->db->link, $admin_email);
        $admin_pass  = mysqli_real_escape_string($this->db->link, $admin_pass);
        $admin_type  = mysqli_real_escape_string($this->db->link, $admin_type);
        $admin_event = mysqli_real_escape_string($this->db->link, $admin_event);
        $admin_email = filter_var($admin_email, FILTER_SANITIZE_EMAIL);

        if (empty($admin_email) || empty($admin_pass) || empty($admin_type) || empty($admin_event)) {
          $msg = '<div class="alert alert-warning" role="alert">
                  Field Must not be Empty!
                 </div>';
          return $msg;
          ...
          $msg = '<div class="alert alert-danger" role="alert">
           ...
           $admin_pass = md5($admin_pass);
           $query = "SELECT * FROM tbl_admin WHERE email = '$admin_email' AND password = '$admin_pass' AND type = 1 AND event_id = '$admin_event'";
           $result = $this->db->select($query);
           if ($result != false) {
              $value = $result->fetch_assoc();
              Session::set("login", true);
              ...
              Session::set("name", $value['name']);
              Session::set("admin_email", $value['email']);
              Session::set("event_id", $value['event_id']);
              Session::set("admin_type", $value['type']);
              Session::set("admin_ck", "event_admin");
	...
	public function getAllPerticpents($event_id){
	    ...
	    /*$sql = "SELECT * FROM `participants` WHERE `event_id`=$event_id AND `payment_status`=1";*/
    ...
    public function addAdminData($name,$email,$password,$type,$admin_event){
        ...
        if (empty($name) || empty($email) || empty($password) || empty($type)) {
         echo '<div class="alert alert-warning" role="alert">
          ...
          echo '<div class="alert alert-danger" role="alert">
            ...
            $sql = "INSERT INTO tbl_admin (name,email,password,type,event_id) VALUES ('$name','$email','$password','$type','$admin_event')";
